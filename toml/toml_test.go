...
import (
	...
	"github.com/nirnanaaa/asparagus/run"
	itoml "github.com/nirnanaaa/asparagus/toml"
)
...
func TestConfig_Encode(t *testing.T) {
	...
	c.Metrics.Cloudwatch.ReportingInterval = itoml.Duration(time.Minute)
	buf := new(bytes.Buffer)
	if err := toml.NewEncoder(buf).Encode(&c); err != nil {
		t.Fatal("Failed to encode: ", err)
	}
	got, search := buf.String(), `reporting-interval = "1m0s"`
